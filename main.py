...
def _selected_stages() -> list[str]:
    ...
    return read_stages(event.resource_config.selector)


def _keep_report(report: dict, stages: list[str]) -> bool:
...
async def on_resync_components(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    include_remediation = read_include_remediation(event.resource_config.selector)
