...
def _event(payload: dict, webhook_id: str) -> WebhookEvent:
    return WebhookEvent(
        trace_id="t1",
        payload=payload,
...
async def test_org_processor_only_handles_organization_owner() -> None:
    org_event = _event({"owner": {"type": "ORGANIZATION", "id": "org-1"}}, "iq:policyManagement")
    app_event = _event({"owner": {"type": "APPLICATION", "id": "app-1"}}, "iq:policyManagement")
...
async def test_authenticate_without_secret_accepts_valid_header(has_secret: bool) -> None:
    event = _event(APP_EVAL_PAYLOAD, "iq:applicationEvaluation")
    processor = ReportWebhookProcessor(event)
